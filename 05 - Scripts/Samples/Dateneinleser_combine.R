...
source("05 - Scripts/Samples/Titel_aus_Datei_combine.R", local = TRUE) 
if(Ierror==0)
{
  for(i in 1:Total.Samples)
  {
    if(Ierror==0)
    {
      ...
      Ierror <- tryCatch(
        {
          Datensatz <- read.csv(paste("01 - Projekte/",Projekt,"/",Charge,"/Messwerte/",get(Title.Sample.all[i]),".csv",sep=""), header=TRUE, dec=",", sep=";",fileEncoding = "ISO_8859-1")
          dim <- dim(Datensatz)
          if(dim[2]<2)
          {
            Datensatz <- read.csv(paste("01 - Projekte/",Projekt,"/",Charge,"/Messwerte/",get(Title.Sample.all[i]),".csv",sep=""), header=TRUE, dec=".", sep=",",fileEncoding = "ISO_8859-1")
      ...
      if(Ierror==1)
      {
        Ierror <- tryCatch(
          {
            Datensatz <- read.csv(paste("01 - Projekte/",Projekt,"/",Charge,"/Messwerte/",get(displayed.Title.Sample.all[i]),".csv",sep=""), header=TRUE, dec=",", sep=";",fileEncoding = "ISO_8859-1")
            dim <- dim(Datensatz)
            if(dim[2]<2)
            {
              Datensatz <- read.csv(paste("01 - Projekte/",Projekt,"/",Charge,"/Messwerte/",get(displayed.Title.Sample.all[i]),".csv",sep=""), header=TRUE, dec=".", sep=",",fileEncoding = "ISO_8859-1")
