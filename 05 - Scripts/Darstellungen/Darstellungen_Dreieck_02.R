...
##funktioniert als Loop noch nicht richtig 
#Voreinstellungen
...
plot.y <- "Si"
plot.z <- "Al"
plot.zz <- "DAVE" #Variable, die im Farbverlauf dargestellt wird
figure.rows <- 4 #Anzahl an Abbildungen pro Spalte 
figure.col <- 3
x.Achse <- paste(plot.x,"-Gehalt in At.-%",sep = "")
y.Achse <- paste(plot.y,"-Gehalt in At.-%",sep = "")
z.Achse <- paste(plot.z,"-Gehalt in At.-%",sep="") #muss angepasst werden, falls in z keine Gehalt dargestellt wird 
Title.legend <- paste(plot.zz,"-Gehalt in At.-%",sep="")
library(ggtern)
library(ggplot2)
a <- 0
b <- 0
...
for(j in 1:Total.Plots)
{
  ...
  ggtern(data=Sample, aes(x=get(plot.x), y=get(plot.y), z=get(plot.z)))+geom_point(aes(fill=Sample[,plot.zz]),size=2,shape=21, color="black")+ggtitle(Title.Sample)+labs(fill=Title.legend)+theme_rgbw()+ theme(plot.title = element_text(hjust=0.5))
  ...
  if(a==(figure.col*figure.rows))
  {
    ...
    savePlot(filename = paste("Ergebnisse/",Charge,"/",Version,"/Darstellungen/",plot.x,"_vs_",plot.y,"_vs_",plot.z,"_Sample_",b-a+1,"-",b,"_Dreieck.pdf",sep = ""), type = "pdf");
    a <- 0
  }
}
if(a!=0)
