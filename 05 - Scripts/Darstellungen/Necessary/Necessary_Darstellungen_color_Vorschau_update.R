...
#durch Umstellungen in alten Datensätzen comined Datei vielleicht nicht vorhanden, deshalb zwei Optionen 
Ierror <- tryCatch(
  {
    i <- 1
    SampleNr <- Samples[i]
        ...
        if(plot.z=="DAVE")
        {
          ...
          if(plot.z=="AREA")
          {
            ...
            if(plot.z=="Dequi")
            {
              ...
              Inhalt.Legende <- c(Inhalt.Legende,paste("Helligkeitsverlauf:"),paste(plot.z,"-Anteil = 0 Ma.-%",sep=""), paste(plot.z,"-Anteil = ",Maximum," Ma.-%",sep=""))
