...
test <- read.csv2("test5.csv")
test <- test[,-1]

#Partikelgröße_nach_Klassen <- Summary.Partikelgröße.nach.Klassen.norm
...
if(dim[1]>0&dim[2]>0)
{
  for(i in 1:dim[1])
  {
    ...
    if(sumall>10)
    {
      ...
      sub_1st_min <- findmin
      if(Summenfunktion[i,min1$V2]<0.1)
      {
        sub_1st_min <- sub_1st_min[-(1:min1$V2),] 
        min2 <-  sub_1st_min[which.min(sub_1st_min$V1),]
      }else{
        if(min1$V2!=1)
        {
          max <- max(findmin$V2)
          sub_1st_min <- sub_1st_min[-(min1$V2:max),]  
          min2 <-  sub_1st_min[which.min(sub_1st_min$V1),]
        }else{
          min2$V2 <-0
        }
      ...
      if(min2$V2==0)
      {
        m <- (get(Klassen.Grenzen[min1$V2+1])-get(Klassen.Grenzen[min2$V2+1]))/(Summenfunktion[i,min1$V2]-0)
      }else{
        m <- (get(Klassen.Grenzen[min1$V2+1])-get(Klassen.Grenzen[min2$V2+1]))/(Summenfunktion[i,min1$V2]-Summenfunktion[i,min2$V2])
      }  #m <- 1
      ...
      if(m=="-Inf"|m=="-Inf")
      {
        ...
        n <- (get(Klassen.Grenzen[min1$V2+1]) - (m*(Summenfunktion[i,min1$V2])))
        d10 <- m*0.1+n
        if(d10<0)
        {
      ...
      #sub_1st_min <- findmin[-(which.min(findmin$V1)),]
      sub_1st_min <- findmin
      if(Summenfunktion[i,min1$V2]<0.5)
      {
        sub_1st_min <- sub_1st_min[-(1:min1$V2),]
        min2 <-  sub_1st_min[which.min(sub_1st_min$V1),]
      }else{
      ...
      #sub_1st_min <- findmin[-(which.min(findmin$V1)),]
      sub_1st_min <- findmin
      if(Summenfunktion[i,min1$V2]<0.9)
      {
        sub_1st_min <- sub_1st_min[-(1:min1$V2),]  
        min2 <-  sub_1st_min[which.min(sub_1st_min$V1),]
      }else{
...
#safewrite.csv(Summary_Kenngrößen, filename = paste("01 - Projekte/",Projekt,"/",Charge,"/Ergebnisse/",Version,"/Partikelgröße/nach Klassen/",aktuelle.Klasse,"_Kenngrößen.csv",sep=""),rownames = TRUE)
